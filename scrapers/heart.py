...
def scrape_heart():
    ...
    for url in all_coffee_links:
        ...
        description = coffee_soup.find(
            'div', {'class': 'tab-content small'}).find(
                'div', {'id': 'tab1'}).text.encode('utf-8').strip()
        notes = coffee_soup.find(
            'p', {'class': 'small uppercase flavors'}).text.split(',')
        if not blend:
    ...
    logging.info('Heart New Results:{} / {}'.format(coffees_entered,
                                                    total_coffees))
    logging.info('Heart Updated Results:{} / {}'.format(coffees_updated,
                                                        total_coffees))
    if error_coffees:
